...
<#
    Deterministic stand-in for New-A365AgentIdentity.ps1's update mode, used only by
    SecureAuthenticationInput.Tests.ps1 (via Update-A365AgentIdentity.ps1 -ScriptRoot
    pointing here). Records enough about -ClientSecret, -CertificatePassword and -AccessToken
    to prove that Update-A365AgentIdentity.ps1 forwards the exact object it was given - never
    a copy, a re-typed value, or a stringified one - without making any Graph call.
#>
...
[pscustomobject]@{ agentIdentityId = $AgentIdentityId; updated = $true }
