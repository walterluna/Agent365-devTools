<#
.SYNOPSIS
    Deletes Microsoft Agent 365 agent users, and nothing else.

.DESCRIPTION
    Removes the agent user directory object, using Microsoft Graph only. No a365 CLI.

    SINGLE RESPONSIBILITY. This script deletes agent users. It does NOT delete the agent
    identity that owns the user, the blueprint, or the agent's registration in the Microsoft
    365 admin center. Each of those has its own script:

        Remove-A365AgentIdentity.ps1      the agent identity
        Remove-A365Blueprint.ps1          the blueprint
        Remove-A365AgentRegistration.ps1  the admin-center registration

    DESTRUCTIVE, and more so than it first appears: an agent user is a real directory object,
    so deleting it destroys the agent's mailbox and releases its user principal name. The UPN
    becomes available for reuse, which a soft delete does not protect you from if something
    else claims it first.

    Deleting is a SOFT delete: the object moves to /directory/deletedItems and is restorable
    for 30 days, and until it is purged it still appears under deleted users in the admin
    centre. -Permanent purges it, which is irreversible.

    Delete the agent user BEFORE its identity. The user is discovered through the identity, so
    removing the identity first leaves the user orphaned and harder to find.

.PARAMETER AgentUserId
    One or more agent user object ids to delete. A user principal name is also accepted.

.PARAMETER Permanent
    After the soft delete, also purge from /directory/deletedItems. IRREVERSIBLE. Without it
    the deleted account still appears under deleted users in the admin centre.

.PARAMETER InspectOnly
    Resolve and report every target, then exit without deleting anything.

.PARAMETER AllowNonAgentUser
    Permit deleting a user that is not an agent user. Off by default, so pointing this script
    at an ordinary employee account by mistake is refused.
...
.EXAMPLE
    .\Remove-A365AgentUser.ps1 -AgentUserId a2a52f41-a150-4cce-98ad-462ac636c478 `
        -Interactive -InspectOnly

    Report what would be deleted, and delete nothing.

.EXAMPLE
    .\Remove-A365AgentUser.ps1 -AgentUserId $id -Permanent -Force -Interactive

    Delete the agent user and purge it from deleted items. IRREVERSIBLE, and frees the UPN.

.NOTES
    Requires AgentIdUser.ReadWrite.All. User.ReadWrite.All does NOT authorize operations on
    agent users.

    Deletion goes to /beta/users/{id}. The /beta/agentUsers/{id} path used by the a365 CLI
    reports success while leaving the user in place.
#>
...
$null = Initialize-A365Log -Path $LogPath -ScriptName 'Remove-A365AgentUser.ps1' `
...
$delegatedScopeSets = @(
    @('AgentIdUser.ReadWrite.All', 'Directory.ReadWrite.All', 'User.ReadWrite.All'),
    @('AgentIdUser.ReadWrite.All', 'Directory.ReadWrite.All'),
...
# ---------------------------------------------------------------------------
Write-Step 2 'Resolving agent users'

$plan = [System.Collections.Generic.List[object]]::new()

foreach ($rawId in $AgentUserId) {
    $id = [string]$rawId
    if ([string]::IsNullOrWhiteSpace($id)) { continue }

    $user = Invoke-Graph -Method GET -Uri "/users/$([uri]::EscapeDataString($id))`?`$select=id,displayName,userPrincipalName" `
    ...
    if ($null -eq $user) {
        ...
        Write-A365Log -Level WARN -Message "agent user $id : $note"
        continue
    }

    $objectId = [string](Get-Value $user 'id' $id)
    $upn      = [string](Get-Value $user 'userPrincipalName' '')
    $label    = [string](Get-Value $user 'displayName' '')

    # Membership of the typed collection is the reliable test, as it is for agent identities.
    # Deleting an ordinary employee account by mistake is not recoverable once purged, and the
    # UPN is released either way, so this is refused unless explicitly allowed.
    $typed = Invoke-Graph -Method GET `
                -Uri "/users/microsoft.graph.agentUser/$objectId`?`$select=id,identityParentId" `
                -TolerateNotFound -TolerateForbidden -TolerateBadRequest

    $isAgentUser = ($null -ne $typed)
    $detail = if ($isAgentUser) {
        "$upn, identity $([string](Get-Value $typed 'identityParentId' 'unknown'))"
    } else { $upn }

    if (-not $isAgentUser -and -not $AllowNonAgentUser) {
        $plan.Add([pscustomobject]@{ Id = $objectId; Label = $label; Detail = $detail; State = 'Skipped'
                                     Note = 'Not an agent user. Pass -AllowNonAgentUser to delete it anyway.' })
        Write-A365Log -Level WARN -Message "agent user $objectId skipped: not an agent user"
        continue
    }

    $note = if ($isAgentUser) { '' } else { 'Not an agent user; allowed by -AllowNonAgentUser.' }
    $plan.Add([pscustomobject]@{ Id = $objectId; Label = $label; Detail = $detail; State = 'Present'; Note = $note })
}
...
if ($deletable.Count -gt 0) {
    Write-Host '  Deleting an agent user destroys its mailbox and releases its UPN.' -ForegroundColor Yellow
}

if ($InspectOnly) {
    Write-Host ''
    Write-Host 'Inspect only - nothing was deleted.' -ForegroundColor Yellow
    Write-A365Log -Level INFO -Message "inspect only: $($deletable.Count) agent user(s) would be deleted"
    if ($PassThru) { $plan }
...
foreach ($item in $deletable) {
    ...
    if (-not $PSCmdlet.ShouldProcess("agent user '$label' ($($item.Id))", 'Delete')) { continue }

    try {
        # /beta/users/{id}, NOT /beta/agentUsers/{id}: the latter reports success and leaves
        # the user in place, which is how the a365 CLI leaves orphans behind.
        $null = Invoke-Graph -Method DELETE -Uri "/users/$($item.Id)" -TolerateNotFound
        ...
        $failures.Add($item)
        Write-A365Log -Level ERROR -Message "delete failed for agent user $($item.Id)" -Detail $_.Exception.Message
...
if ($Permanent) {
    ...
    foreach ($item in $deletable) {
        ...
        if (-not $PSCmdlet.ShouldProcess("agent user $($item.Id) in deleted items", 'Purge permanently')) { continue }
        try {
...
Write-Host ' Agent user removal complete' -ForegroundColor Green
Write-Host '========================================================' -ForegroundColor Green
